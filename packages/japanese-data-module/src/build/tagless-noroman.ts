...
import type {KanjiRow, WordRow} from '../tagless-noroman/index.js';

const TARGET_PATH = pathlib.join(_TARGET_PATH, 'tagless-noroman');

export async function buildTaglessNoRoman() {
...
async function buildKanjiFiles() {
	...
	for (; i < 5; ++i) {
		...
		for (const obj of data) {
			structure.push([obj.id, jlpt, obj.v, obj.meaning]);
		}
